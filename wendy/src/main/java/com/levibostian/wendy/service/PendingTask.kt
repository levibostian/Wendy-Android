 ...
 *
 * @property taskId ID of the [PendingTask]. After you have used [PendingTasks.addTask] to add this task to Wendy, this property will become populated and available to you. It is then up to *you* to hang onto this ID if you want to reference it later on.
 * @property createdAt The date/time that the task was created.
 * @property manuallyRun Sometimes you may want your user to be in charge of when a task is run. Setting [manuallyRun] to true will assert that this task does not get run automatically by the Wendy task runner. You will need to manually run the task yourself via [PendingTasks.runTask].
